import torch 
import torch.nn as nn
import os
import copy
import pathlib
import cv2
import numpy as np
import warnings

from models.common import Conv, DetectMultiBackend
from utils.general import check_img_size, non_max_suppression #, scale_coords, scale_coords_landmarks
from utils.datasets import letterbox
from utils.torch_utils import select_device

# TODO - ADD RELATIVE PATH for weights

#class Detector : classe contenant toutes les fonctions nécessaires au bon fonctionnement du module de detection
class Detector:
    def __init__(self, weights_name, device, min_face, target_size):
        """
        weights_name: name of file with network weights in weights/ folder.
        config_name: name of .yaml config with network configuration from models/ folder.
        gpu : pytorch device. Use 'cuda:0', 'cuda:1', e.t.c to use gpu or 'cpu' to use cpu.
        min_face : minimal face size in pixels.
        target_size : target size of smaller image axis (choose lower for faster work). e.g. 480, 720, 1080. Choose None for original resolution.
        """
        self._class_path = pathlib.Path(__file__).parent.absolute()#os.path.dirname(inspect.getfile(self.__class__))
        self.device = device
        self.target_size = target_size
        self.min_face = min_face
        self.detector = self.init_detector(weights_name)

    
    #Fonction init_detector : permet d'initialiser le module de detection 
    #Entrée : self, weights_name = nom des poids du modèle
    #Sortie : detector
    def init_detector(self,weights_name):
        print("device : " + self.device)
        
        model_path = os.path.join(self._class_path,'../weights/',weights_name)
        print("model : " + model_path)
        
        device = select_device(self.device)
        detector = DetectMultiBackend(weights=model_path, device=device)
        
        for m in detector.modules():
            if type(m) in [nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU]:
                m.inplace = True  # pytorch 1.7.0 compatibility
            elif type(m) is Conv:
                m._non_persistent_buffers_set = set()  # pytorch 1.6.0 compatibility
       
        return detector

    #Fonction __preprocess : premier traitement des images avant la detection
    #Entrée : self, imgs = les images à traiter
    #Sortie : pp_imgs = images pré-traitées
    def __preprocess(self,imgs):
        """
            Preprocessing image before passing through the network. Resize and conversion to torch tensor.
        """
        pp_imgs = []
        for img in imgs:
            h0, w0 = img.shape[:2]  # orig hw
            if self.target_size:
                r = self.target_size / min(h0, w0)  # resize image to img_size
                if r < 1:  
                    img = cv2.resize(img, (int(w0 * r), int(h0 * r)), interpolation=cv2.INTER_LINEAR)

            imgsz = check_img_size(max(img.shape[:2]), s=self.detector.stride)  # check img_size (stride.max())
            img = letterbox(img, new_shape=imgsz)[0]
            pp_imgs.append(img)
        pp_imgs = np.array(pp_imgs)
        pp_imgs = pp_imgs.transpose(0, 3, 1, 2)
        pp_imgs = torch.from_numpy(pp_imgs).to(self.device)
        pp_imgs = pp_imgs.float()  # uint8 to fp16/32
        pp_imgs /= 255.0  # 0 - 255 to 0.0 - 1.0
        return pp_imgs

    #Fonction __postprocess : 
    #Entrée : self, imgs = les images à traiter, origimgs, pred, conf_thres, iou_thres
    #Sortie : bboxes = boite de detection des éléments
    def __postprocess(self, imgs, origimgs, pred, conf_thres, iou_thres):
        """
            Postprocessing of raw pytorch model output.
            Returns:
                bboxes: list of arrays with 4 coordinates of bounding boxes with format x1,y1,x2,y2.
                points: list of arrays with coordinates of 5 facial keypoints (eyes, nose, lips corners).
        """
        bboxes = [[] for i in range(len(origimgs))]
        
        pred = non_max_suppression(pred, conf_thres, iou_thres)
        
        for i in range(len(origimgs)):
            img_shape = origimgs[i].shape
            h,w = img_shape[:2]
            gn = torch.tensor(img_shape)[[1, 0, 1, 0]]  # normalization gain whwh
            det = pred[i].cpu()
            # scaled_bboxes = scale_coords(imgs[i].shape[1:], det[:, :4], img_shape).round()
            # scaled_cords = scale_coords_landmarks(imgs[i].shape[1:], det[:, 5:15], img_shape).round()

            for j in range(det.size()[0]):
                box = (det[j, :4].view(1, 4) / gn).view(-1).tolist()
                box = list(map(int,[box[0]*w,box[1]*h,box[2]*w,box[3]*h]))
                if box[3] - box[1] < self.min_face:
                    continue
                bboxes[i].append(box)

        return bboxes

    #Fonction predict : 
    #Entrée : self, imgs = les images à traiter, conf_thres, iou_thres
    #Sortie : bboxes = boite de detection des éléments
    def predict(self, imgs, conf_thres, iou_thres):
        '''
            Get bbox coordinates and keypoints of faces on original image.
            Params:
                imgs: image or list of images to detect faces on
                conf_thres: confidence threshold for each prediction
                iou_thres: threshold for NMS (filtering of intersecting bboxes)
            Returns:
                bboxes: list of arrays with 4 coordinates of bounding boxes with format x1,y1,x2,y2.
        '''
        one_by_one = False
        # Pass input images through face detector
        if type(imgs) != list:
            images = [imgs]
        else:
            images = imgs
            one_by_one = False
            shapes = {arr.shape for arr in images}
            if len(shapes) != 1:
                one_by_one = True
                warnings.warn(f"Can't use batch predict due to different shapes of input images. Using one by one strategy.")
        origimgs = copy.deepcopy(images)
        
        
        if one_by_one:
            images = [self.__preprocess([img]) for img in images]
            bboxes = [[] for i in range(len(origimgs))]
            points = [[] for i in range(len(origimgs))]
            for num, img in enumerate(images):
                with torch.inference_mode(): # change this with torch.no_grad() for pytorch <1.8 compatibility
                    single_pred = self.detector(img)[0]
                    print(single_pred.shape)
                bb, pt = self.__postprocess(img, [origimgs[num]], single_pred, conf_thres, iou_thres)
                #print(bb)
                bboxes[num] = bb[0]
                points[num] = pt[0]
        else:
            images = self.__preprocess(images)
            with torch.inference_mode(): # change this with torch.no_grad() for pytorch <1.8 compatibility
                pred = self.detector(images)[0]
            bboxes = self.__postprocess(images, origimgs, pred, conf_thres, iou_thres)

        return bboxes

    def __call__(self,*args):
        return self.predict(*args)


#class NanoFace : classe utilisé pour la detection, utilise predict pour detecter l'objet
class NanoFace(Detector):
    def __init__(self):
        super(NanoFace,self).__init__( \
            weights_name='face_n.pt',  \
            device='cpu',           \
            min_face=0,                \
            target_size=None)           \
    
    def predict(self, imgs, conf_thres = 0.3, iou_thres = 0.5):
        return super(NanoFace,self).predict(imgs,conf_thres,iou_thres)

